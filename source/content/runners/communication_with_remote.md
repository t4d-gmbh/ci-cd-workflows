...
{% else %}
Runners communicate with remote services via HTTP/HTTPS and may require access tokens to fetch protected resources.
...
With these tokens, the runner can perform edits in a <i class="fab fa-gitlab"></i> Project or <i class="fab fa-github"></i> Repository, whether in the actual <i class="fab fa-git"></i> repository or in associated features of the remote service, such as Issues or Merge/Pull Requests.
...
This has significant implications for the network accessibility of a runner: while it requires outgoing connections, the runner does not need to be reachable from the internet, allowing it to reside in a semi-isolated environment.
{% endif %}
